...
class ContractCreateFlow {
  ...
  async executeWithSigner(signer) {
    ...
    const fileCreateTransaction = new _FileCreateTransaction.default().setKeys(key != null ? [key] : []).setContents(this._bytecode.subarray(0, Math.min(this._bytecode.length, 2048)));
    ...
    if (this._bytecode.length > 2048) {
      const fileAppendTransaction = new _FileAppendTransaction.default().setFileId(fileId).setContents(this._bytecode.subarray(2048));
    ...
    if (key != null) {
      const fileDeleteTransaction = new _FileAppendTransaction.default().setFileId(fileId);
