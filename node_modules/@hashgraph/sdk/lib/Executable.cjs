  ...
  async execute(client, requestTimeout) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      ...
      try {
        ...
        _jsLogger.default.trace(`[${this._getLogId()}] sending protobuf ${hex.encode(this._requestToBytes(request))}`);
      ...
      _jsLogger.default.trace(`[${this._getLogId()}] sending protobuf ${hex.encode(this._responseToBytes(response))}`); // If we didn't receive an error we should decrease the current nodes backoff
